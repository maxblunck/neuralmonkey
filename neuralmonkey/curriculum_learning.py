...
def sort_data(data_series, vocabulary, criterion='sent_len', level='word', side='target', thresholds=None, num_bins=5, normalize=False):
    ...
    if criterion == 'sent_len':
        ...
        bins = _bins_by_vocab_rank(zipped, vocabulary, level, side, thresholds=thresholds, num_bins=num_bins, normalize=normalize)
...
def _bins_by_vocab_rank(parallel_corpus, vocab_path, level, side, normalize, thresholds=None, num_bins=None, print_stats=True):
    """
    ...
    if side == "source":
        ...
        if level == "word":
            sorted_dataset = sorted(parallel_corpus, key=lambda x: _get_sent_rank(x[0], word_to_rank, thresholds, normalize=normalize))
        elif level == "subword":
            sorted_dataset = sorted(parallel_corpus, key=lambda x: _get_sent_rank(x[2], word_to_rank, thresholds, normalize=normalize))

    elif side == "target":

        if level == "word":
            sorted_dataset = sorted(parallel_corpus, key=lambda x: _get_sent_rank(x[1], word_to_rank, thresholds, normalize=normalize))
        elif level == "subword":
            sorted_dataset = sorted(parallel_corpus, key=lambda x: _get_sent_rank(x[3], word_to_rank, thresholds, normalize=normalize))
    ...
    if print_stats == True:
        ...
        if thresholds == None:
            for bin in bins:
                ...
                for pair in bin:
                    ...
                    # here value is a worst rank average of the bin, instead of minimum vocab frequency!
                    elif normalize == True:
                        min_b_freq = 1
                        av_sent_rank = _get_sent_rank(sent, word_to_rank, normalize=normalize)
                        if av_sent_rank > min_b_freq:
                            min_b_freq = av_sent_rank
        ...
        for i in range(len(bins)):
            if level == "word":
                example = random.choice(bins[i])
                log("\nBin {}:\n size: {}\n example: \n {} \n ---> \n {}\n".format(i, len(bins[i]), " ".join(example[0]), " ".join(example[1])))
                #log("Target rank: {}".format(_get_sent_rank(example[1], word_to_rank, normalize=normalize)))
            elif level == "subword":
                example = random.choice(bins[i])
                log("\nBin {}:\n size: {}\n example: \n{} \n ---> \n {}\n".format(i, len(bins[i]), " ".join(example[2]), " ".join(example[3])))
    return bins


def _get_sent_rank(sentence, word_to_rank, thresholds=None, normalize=False):
    if normalize == False:
        max_rank = 0
        ...
        for word in sentence:
            try:
                ...
                word_rank = len(word_to_rank.values())
            #print("{}: {}".format(word, word_rank))
            rank_sum += word_rank

        average_rank = rank_sum / len(sentence)
        #print("average:")
        #print(average_rank)
        return int(average_rank)
